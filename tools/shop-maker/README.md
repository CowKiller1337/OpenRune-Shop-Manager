...
- **Recreate existing** only lists Trade NPCs that are already spawned in the world and already have
  shop data. This changes the shop title, stock, and prices; if the NPC is missing a native Trade
  script, it writes a normal Aubury-style `PluginScript` for that NPC. It does not move the NPC.
  Make a new custom shop if you want different coordinates.
...
When placing a brand-new custom shop, the tool checks one server-side support piece:
...
before updating the cache packer source file. Existing-shop edits do not need this patch. If the
file layout does not match the expected OpenRune source, the tool refuses to patch it. The tool does
not install a custom shop runtime handler; repaired and newly placed shops use generated native
OpenRune scripts that call `shops.open(...)`.
...
Existing shop edits update the native `inv.*` shop TOML directly and can repair the NPC's native
Trade script when one is missing.
